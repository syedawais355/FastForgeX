"""Verify built wheel package files exactly match repository source files."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare latest wheel package files with local fastforgex source tree."
    )
...
def main() -> int:
    ...
    with zipfile.ZipFile(wheel_path) as wheel:
        ...
        if missing_in_wheel or extra_in_wheel:
            if missing_in_wheel:
                print("Files missing in wheel:")
                for item in missing_in_wheel:
                    print(f"  - {item}")
            if extra_in_wheel:
                print("Unexpected files present in wheel:")
        ...
        for rel, repo_path in sorted(repo_files.items()):
            ...
            if repo_bytes != wheel_bytes:
                mismatches.append(f"{rel}: repo={_sha256(repo_bytes)} wheel={_sha256(wheel_bytes)}")

    if mismatches:
        print("Content mismatches found between source and wheel:")
        for mismatch in mismatches:
            print(f"  - {mismatch}")
        return 1

    print(f"Verified wheel source parity: {wheel_path.name}")
    return 0
